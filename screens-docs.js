...
function openAdd(){
  haptic();
  sheet(`
    <h3>Shto dokument</h3><div class="sub">Zgjidh mënyrën — AI e njeh automatikisht.</div>
...
async function runOCR(blob,name,scanned){
  ...
  const cat=guessCategory(text||name).cat;
  const type=guessType(text||name,cat);
...
function addManual(){
  ...
  const id=uid();
  sheet(`
    <h3>Shto manualisht</h3><div class="sub">Plotëso të dhënat e dokumentit.</div>
    <div class="field"><label>Titulli</label><input class="input" id="ad_title" placeholder="p.sh. Pasaportë"/></div>
    <div class="row" style="gap:10px">
      <div class="field" style="flex:1"><label>Kategoria</label><select class="input" id="ad_cat">${catKeys.map(k=>`<option value="${k}">${CATS[k].name}</option>`).join('')}</select></div>
      <div class="field" style="flex:1"><label>Lloji</label><input class="input" id="ad_type" placeholder="Lloji"/></div>
    </div>
    <div class="field"><label>Institucioni</label><input class="input" id="ad_inst"/></div>
    <div class="row" style="gap:10px">
    ...
    <button class="btn primary" onclick="saveNewDoc('${id}')">${svg('check',18)} Ruaj</button>
    <button class="btn ghost mt" onclick="closeSheet()">Anulo</button>
  `);
}
